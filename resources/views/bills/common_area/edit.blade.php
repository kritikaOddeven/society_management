@extends('layouts.app')
@section('pagetitle', 'Edit Common Area Bill')
@section('main-content')
    {{-- Main section --}}
    <section class="section">
        <div class="section-header">
            <h1>Edit Common Area Bill</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">Dashboard</a></div>
                <div class="breadcrumb-item"><a href="{{ route('bills.common_area.index') }}">Common Area Bills</a></div>
                <div class="breadcrumb-item">Edit Common Area Bill</div>
            </div>
        </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>Edit Common Area Bill</h4>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('bills.common_area.update', $bill) }}" method="POST" enctype="multipart/form-data" id="editCommonAreaBillForm">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="bill_type_id">Bill Type <span class="text-danger">*</span></label>
                                            <select class="form-control @error('bill_type_id') is-invalid @enderror" id="bill_type_id" name="bill_type_id">
                                                <option value="">Select Bill Type</option>
                                                @foreach ($billTypes as $billType)
                                                    <option value="{{ $billType->id }}" {{ old('bill_type_id', $bill->bill_type_id) == $billType->id ? 'selected' : '' }}>
                                                        {{ $billType->bill_type }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            ...
                                            @enderror
                                            <span class="text-danger bill_type_id-error"></span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control @error('bill_amount') is-invalid @enderror" id="bill_amount" name="bill_amount" value="{{ old('bill_amount', $bill->bill_amount) }}" placeholder="Enter amount">
                                            @error('bill_amount')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control @error('payment_mode') is-invalid @enderror" id="payment_mode" required name="payment_mode">
                                                ...
                                                @foreach ($paymentModes as $value => $label)
                                                    <option value="{{ $value }}" {{ old('payment_mode', $bill->payment_mode) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                                @endforeach
                                            </select>
                                            @error('payment_mode')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control flatpickr flatpickr-date @error('bill_date') is-invalid @enderror" id="bill_date" name="bill_date" value="{{ old('bill_date', optional($bill->bill_date)->format('Y-m-d')) }}" placeholder="Bill date">
                                            @error('bill_date')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control flatpickr flatpickr-date @error('bill_due_date') is-invalid @enderror" id="bill_due_date" name="bill_due_date" value="{{ old('bill_due_date', optional($bill->bill_due_date)->format('Y-m-d')) }}" placeholder="Bill due date">
                                            @error('bill_due_date')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                    ...
                                    <div class="col-md-6" id="bill_image_field">
                                        <div class="form-group">
                                            ...
                                            @if ($bill->bill_image)
                                                <div class="mt-2">
                                                    <a href="{{ asset($bill->bill_image) }}" target="_blank" class="btn btn-sm btn-info">
                                ...
                                <div class="form-group d-flex justify-content-end">
                                    <a href="{{ route('bills.common_area.index') }}" class="btn btn-secondary mr-2">Cancel</a>
